...
let rec evalExp (e : UntypedExp, vtab : VarTable, ftab : FunTable) : Value =
  ...
  | And (e1, e2, pos) ->
        ...
        let res2 = evalExp(e2, vtab, ftab)
        match (res1, res2) with
            | (BoolVal n1, BoolVal n2) ->
                match (n1,n2) with
                    | (true,true) -> BoolVal(true)
                    | _ ->           BoolVal(false)
            | _ -> invalidOperands "And.....: " [(Bool, Bool)] res1 res2 pos
  | Or (e1, e2, pos) ->
        let res1 = evalExp(e1, vtab, ftab)
        let res2 = evalExp(e2, vtab, ftab)
        match (res1, res2) with
            | (BoolVal n1, BoolVal n2) ->
                match (n1,n2) with
                    ...
                    | (_,true) -> BoolVal(true)
                    | _ ->        BoolVal(false)
            | _ -> invalidOperands "Or.....: " [(Bool, Bool)] res1 res2 pos
  | Not(e1, pos) ->
        let res1 = evalExp(e1, vtab, ftab)
        match (res1) with
            | (BoolVal n1) ->
                match (n1) with
                    | true -> BoolVal(false)
                    | _ -> BoolVal(true)
            | _ -> invalidOperand "Not.....: " Bool res1 pos
  | Negate(e1, pos) ->
        let res1 = evalExp(e1, vtab, ftab)
        match (res1) with
              | (IntVal n1) -> IntVal(-1*n1)
              | _ -> invalidOperand "Negate.....: " Int res1 pos
  ...
  | Filter (farg, arr_exp, _, pos) ->
        ...
        match arr_val with
            | ArrayVal(lst, tp) ->
                ...
                ArrayVal(res_lst, tp)
            |_ -> raise (MyError("Second argument of filter does not evaluate to an
