...
and checkExp  (ftab : FunTable)
    ...
    *)
    ...
    | And (e1, e2, pos) ->
        ...
        else raise (MyError ("In Bool: one of subexpression types is not Bool: "+ppType t1+" and "+ppType t2, pos))
    ...
    | Not (e1, pos) ->
        ...
        else raise (MyError ("In Not: type is not Bool: "+ppType t1, pos))

    | Negate (e1, pos) ->
        let (t1, e1_dec) = checkExp ftab vtab e1
        if (Int = t1)
        then (Int, Negate (e1_dec, pos))
        else raise (MyError ("In Negate: type is not Int: "+ppType t1, pos))
    ...
    *)
    | Filter (farg, arr_exp, _, pos) ->
        ...
        let el_tp = match arr_tp with
                | Array(e_tp) -> e_tp
                | _ -> raise (MyError( "Filter: expression input not an array" +
                                ppType arr_tp , pos))
        let (rea_tp_fn, inp_tp_fn, dec_farg) =
            match checkFunArg ftab vtab pos farg with
                | (farg_dec, res_tp, [inp_tp]) -> (res_tp, inp_tp, farg_dec)
                | _ -> raise (MyError("Filter: function argument does not take
                exactly on param ", pos))
        if (rea_tp_fn = Bool) && (el_tp = inp_tp_fn)
        then (arr_tp, Filter(dec_farg, dec_arr_exp, el_tp, pos))
        else raise (MyError("Filter: result not a bool or funarg elem type does not
    ...
    *)
    | Scan (farg, e, arr_exp, _, pos) ->
        ...
        if (el_tp=e_tp) then
          let (rea_tp_fn, inp_tp_fn, dec_farg) =
              match checkFunArg ftab vtab pos farg with
                  | (farg_dec, res_tp, [e_tp; inp_tp]) -> (res_tp, inp_tp, farg_dec)
